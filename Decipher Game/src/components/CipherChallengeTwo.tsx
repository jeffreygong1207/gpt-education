...
interface CipherChallengeTwoProps {
  onComplete: (isCorrect: boolean, selectedLetters: Set<number>) => void;
}

export function CipherChallengeTwo({ onComplete }: CipherChallengeTwoProps) {
  const [selectedAnswer, setSelectedAnswer] = useState<boolean | null>(null);
  const [selectedLetters, setSelectedLetters] = useState<Set<number>>(new Set());
  const reference = "The quick brown fox jumps over lazy dogs.";
  const message = "Lsy zqick dgpwh rtg jinph wxg mpty yxft.";
    ...
    <div className="max-w-4xl mx-auto text-center space-y-8 p-6">
      <div className="space-y-6">
        <div className="flex items-center justify-center gap-4">
          <h2 className="text-3xl font-bold text-white">Practice Task 2</h2>
          <CipherInfo />
        </div>
